function [V,Core,info,X,itr] = DCOT_CMU(X,mark,para_ST,Xg,src_type,B_SVD)
...
for itr = 1 : para_ST.maxitr
    ...
    Xg=X-S-Xh;
    G = optimize_l1Z(Z,V,Xg,Y,ita,1e-2);
    ...
    switch src_type
        ...
        %   subplot(1,3,1);imshow(S(:,:,1));title('completed tensor');
            ...
            pause(0.1);
        case 'CMU'
            subplot(1,2,2);plot(info.rse);axis([0,para_ST.maxitr,0,inf]);title('# iterations vs. RSEs');
