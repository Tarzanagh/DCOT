function [V,Core,info,X,itr] = DCOT_CMU_OINIT_OTV(X,mark,para_ST,Xg,src_type,B_SVD)
...
if ~isfield(para_ST,'VSet')
    ...
    para_ST.H = myconstruct_graphL(tsize,para_ST.VSet,para_ST.Rate,para_ST.gnns,para_ST.Affinity);
...
switch src_type
    ...
    case 'CMU'
        ...
        subplot(1,3,1);imshow(CC),title('incomplete tensor (1st subject)');
...
for itr = 1 : para_ST.maxitr
    ...
    X_G=X-S-X_H;
    G = optimize_l1Z(Z,V,X_G,Y,ita,1e-2);
    ...
    for i=1:size(H,1)
        ...
        for j=1:size(H,3)
            ...
            H(i,:,j,:)= optimize_l2H(H,[V(2),V(4)],Xhij,Yij,ita, .01*para_ST.gamma);
        end
    end
    H=zeros(size(G));
    Z=G+H;
