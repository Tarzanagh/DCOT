function [V,Core,info,X,itr] = DCOT_CMU_OTV(X,mark,para_ST,Xg,src_type,B_SVD)
...
for itr = 1 : para_ST.maxitr
    ...
    Xg=X-S-Xh;
    G = optimize_l1Z(Z,V,Xg,Y,ita,0.005);
    ...
    for i=1:size(H,1)
        ...
        for j=1:size(H,3)
            ...
            H(i,:,j,:)= optimize_l2H(H,V(2),Xhij,Yij,ita, .25*para_ST.gamma);
        end
    end
    Z=G+H;
    % update X
